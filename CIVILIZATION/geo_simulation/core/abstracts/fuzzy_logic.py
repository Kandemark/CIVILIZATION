...
def create_happiness_fuzzy_system() -> FuzzyInferenceSystem:
    ...
    economy.add_set(FuzzySet("poor", 0, 0, 0.3, 0.5))
    economy.add_set(FuzzySet("average", 0.3, 0.5, 0.7))
    economy.add_set(FuzzySet("strong", 0.5, 0.7, 1, 1))
    
    safety = FuzzyVariable("safety")
    safety.add_set(FuzzySet("low", 0, 0, 0.4, 0.6))
    safety.add_set(FuzzySet("medium", 0.4, 0.6, 0.8))
    safety.add_set(FuzzySet("high", 0.6, 0.8, 1, 1))
    
    # Create output variable
    happiness = FuzzyVariable("happiness")
    happiness.add_set(FuzzySet("low", 0, 0, 0.3, 0.5))
    happiness.add_set(FuzzySet("medium", 0.3, 0.5, 0.7))
    happiness.add_set(FuzzySet("high", 0.5, 0.7, 1, 1))
