...
class SoftMetricsManager:
    ...
    def _setup_fuzzy_variables(self) -> Dict[str, FuzzyVariable]:
        ...
        happiness_var.add_set(FuzzySet("Low", 0, 0, 0.3, 0.5))
        happiness_var.add_set(FuzzySet("Medium", 0.3, 0.5, 0.7))
        happiness_var.add_set(FuzzySet("High", 0.5, 0.7, 1, 1))
        
        # Legitimacy fuzzy sets
        legitimacy_var = FuzzyVariable("Legitimacy")
        legitimacy_var.add_set(FuzzySet("Illegitimate", 0, 0, 0.3, 0.4))
        legitimacy_var.add_set(FuzzySet("Questionable", 0.3, 0.5, 0.7))
        legitimacy_var.add_set(FuzzySet("Legitimate", 0.6, 0.8, 1, 1))
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            'prestige': self.prestige_system.calculate_prestige(),
            'mood': self.happiness_metrics.get_mood().name
        }