...
civ_memory_pool_manager_t* civ_memory_pool_manager_create(size_t default_block_size, size_t default_block_count) {
    civ_memory_pool_manager_t* manager = (civ_memory_pool_manager_t*)CIV_MALLOC(sizeof(civ_memory_pool_manager_t));
    if (!manager) {
    ...
    manager->default_block_size = default_block_size > 0 ? default_block_size : 1024;
    manager->default_block_count = default_block_count > 0 ? default_block_count : 100;
...
static civ_memory_pool_t* create_pool(size_t block_size, size_t block_count) {
    civ_memory_pool_t* pool = (civ_memory_pool_t*)CIV_MALLOC(sizeof(civ_memory_pool_t));
    if (!pool) return NULL;
...
void* civ_memory_pool_allocate(civ_memory_pool_manager_t* manager, size_t size) {
    ...
    size_t block_size = size > manager->default_block_size ? size : manager->default_block_size;
...
void civ_memory_pool_free(civ_memory_pool_manager_t* manager, void* ptr) {
    ...
    civ_memory_pool_t* pool = manager->pools;
    while (pool) {
        if (ptr >= pool->memory && ptr < (char*)pool->memory + (pool->block_size * pool->block_count)) {
