...
void civ_screen_health_render(SDL_Renderer *r, civ_game_t *g, civ_font_t *f,
  ...
  if(pc){snprintf(buf,sizeof(buf),"Health:%.0f%% Cost:%.0f/mo",pc->health,pc->healthcare_cost); uint32_t hc=pc->health>70?0x44FF44:pc->health>40?g_theme.warning:g_theme.danger; civ_font_render_aligned(r,f,buf,lx,dy,w-8,18,hc,CIV_ALIGN_LEFT,CIV_VALIGN_TOP); dy+=24;}
  ...
  for(int i=0;i<5;i++){bool hov=civ_input_is_mouse_over(in,lx,dy,w-8,26); civ_render_rect_filled_alpha(r,lx,dy,w-8,24,hov?0x162033:0x080C18,200); civ_font_render_aligned(r,f,hl[i],lx+10,dy,w-20,24,hov?0xFFFFFF:g_theme.text_secondary,CIV_ALIGN_LEFT,CIV_VALIGN_MIDDLE);
