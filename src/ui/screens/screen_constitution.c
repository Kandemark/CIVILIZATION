...
void civ_screen_constitution_render(SDL_Renderer *r, civ_game_t *g, civ_font_t *f,
  ...
  if(nc){civ_national_constitution_t*c=(civ_national_constitution_t*)nc; snprintf(buf,sizeof(buf),"%s",c->name); civ_font_render_aligned(r,f,buf,lx,dy,w-8,18,g_theme.warning,CIV_ALIGN_LEFT,CIV_VALIGN_TOP); dy+=22;
    for(int rl=0;rl<CIV_RULE_COUNT;rl++){uint32_t pc=c->rules[rl].level==CIV_PERM_PROTECTED_RIGHT?0x44FF44:c->rules[rl].level==CIV_PERM_ALLOWED?0xAACCAA:c->rules[rl].level==CIV_PERM_RESTRICTED?g_theme.warning:g_theme.danger;
      ...
      if(c->rules[rl].cost>0){snprintf(buf,sizeof(buf),"Cost:%.0f Age:%.0f",c->rules[rl].cost,c->rules[rl].minimum_age); civ_font_render_aligned(r,f,buf,lx+w/2,dy,w/2-10,16,g_theme.text_dim,CIV_ALIGN_LEFT,CIV_VALIGN_TOP);} dy+=18;}
  }else{civ_font_render_aligned(r,f,"No constitution data",lx,dy,w-8,20,g_theme.text_dim,CIV_ALIGN_LEFT,CIV_VALIGN_TOP);}
