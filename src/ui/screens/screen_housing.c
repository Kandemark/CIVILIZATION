...
void civ_screen_housing_render(SDL_Renderer *r, civ_game_t *g, civ_font_t *f,
  ...
  if(pc){snprintf(buf,sizeof(buf),"Current: %s (%.0f/mo)",hs[pc->housing_level%4],pc->housing_cost); civ_font_render_aligned(r,f,buf,lx,dy,w-8,18,g_theme.text_secondary,CIV_ALIGN_LEFT,CIV_VALIGN_TOP); dy+=24;}
  ...
  for(int i=0;i<6;i++){bool hov=civ_input_is_mouse_over(in,lx,dy,w-8,26); civ_render_rect_filled_alpha(r,lx,dy,w-8,24,hov?0x162033:0x080C18,200); civ_font_render_aligned(r,f,ha[i],lx+10,dy,w-20,24,hov?0xFFFFFF:g_theme.text_secondary,CIV_ALIGN_LEFT,CIV_VALIGN_MIDDLE); dy+=28;}
