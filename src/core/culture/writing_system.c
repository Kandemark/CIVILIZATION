...
civ_writing_system_manager_t* civ_writing_system_manager_create(void) {
    civ_writing_system_manager_t* manager = (civ_writing_system_manager_t*)CIV_MALLOC(sizeof(civ_writing_system_manager_t));
    if (!manager) {
...
void civ_writing_system_manager_init(civ_writing_system_manager_t* manager) {
    ...
    manager->scripts = (civ_writing_system_t*)CIV_CALLOC(manager->script_capacity, sizeof(civ_writing_system_t));
}

civ_writing_system_t* civ_writing_system_create(const char* id, const char* name, civ_script_type_t type) {
    if (!id || !name) return NULL;
    
    civ_writing_system_t* script = (civ_writing_system_t*)CIV_MALLOC(sizeof(civ_writing_system_t));
    if (!script) {
...
civ_result_t civ_writing_system_set_characters(civ_writing_system_t* script, const char* consonants, const char* vowels) {
...
civ_result_t civ_writing_system_evolve_from(civ_writing_system_manager_t* manager, const civ_writing_system_t* parent, const char* new_id, const char* new_name) {
    civ_result_t result = {CIV_OK, NULL};
    ...
    civ_writing_system_t* new_script = civ_writing_system_create(new_id, new_name, parent->type);
    if (!new_script) {
        result.error = CIV_ERROR_OUT_OF_MEMORY;
        return result;
    }
    
    strncpy(new_script->parent_script_id, parent->id, sizeof(new_script->parent_script_id) - 1);
    ...
    new_script->complexity = CLAMP(parent->complexity + (name_rng_next() % 100 - 50) * 0.01f, 0.0f, 1.0f);
    new_script->efficiency = CLAMP(parent->efficiency + (name_rng_next() % 100 - 50) * 0.01f, 0.0f, 1.0f);
...
char* civ_writing_system_generate_name(const civ_writing_system_t* script, size_t min_length, size_t max_length) {
    if (!script || !script->consonants || !script->vowels) return NULL;
    
    size_t length = min_length + (name_rng_next() % (max_length - min_length + 1));
...
civ_result_t civ_writing_system_manager_add(civ_writing_system_manager_t* manager, civ_writing_system_t* script) {
    civ_result_t result = {CIV_OK, NULL};
    ...
    if (manager->script_count >= manager->script_capacity) {
        ...
        manager->scripts = (civ_writing_system_t*)CIV_REALLOC(manager->scripts,
...
civ_writing_system_t* civ_writing_system_manager_find(const civ_writing_system_manager_t* manager, const char* id) {
