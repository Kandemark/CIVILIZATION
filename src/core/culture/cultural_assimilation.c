...
civ_assimilation_tracker_t* civ_assimilation_tracker_create(void) {
    civ_assimilation_tracker_t* tracker = (civ_assimilation_tracker_t*)CIV_MALLOC(sizeof(civ_assimilation_tracker_t));
    if (!tracker) {
...
void civ_assimilation_tracker_init(civ_assimilation_tracker_t* tracker) {
    ...
    tracker->events = (civ_assimilation_event_t*)CIV_CALLOC(tracker->event_capacity, sizeof(civ_assimilation_event_t));
}

civ_result_t civ_assimilation_tracker_add_event(civ_assimilation_tracker_t* tracker,
                                               const char* source_id, const char* target_id,
                                               const char* region_id, civ_assimilation_type_t type) {
    ...
    if (tracker->event_count >= tracker->event_capacity) {
        ...
        tracker->events = (civ_assimilation_event_t*)CIV_REALLOC(tracker->events,
    ...
    if (tracker->events) {
        ...
        strncpy(event->source_culture_id, source_id, sizeof(event->source_culture_id) - 1);
        strncpy(event->target_culture_id, target_id, sizeof(event->target_culture_id) - 1);
        if (region_id) {
...
civ_result_t civ_assimilation_tracker_update(civ_assimilation_tracker_t* tracker,
    ...
    for (size_t i = 0; i < tracker->event_count; i++) {
        ...
        civ_cultural_identity_t* source = civ_cultural_identity_manager_find(identity_manager, event->source_culture_id);
        civ_cultural_identity_t* target = civ_cultural_identity_manager_find(identity_manager, event->target_culture_id);
        
        if (!source || !target) {
            /* Remove invalid event */
            memmove(&tracker->events[i], &tracker->events[i + 1],
                   (tracker->event_count - i - 1) * sizeof(civ_assimilation_event_t));
            tracker->event_count--;
            i--;
            continue;
        }
        ...
        event->rate = civ_assimilation_calculate_rate(tracker, source, target, event->type);
        
        /* Update progress */
        civ_float_t progress_delta = event->rate * (1.0f - event->resistance) * time_delta;
            ...
            for (size_t j = 0; j < source->trait_count; j++) {
                ...
                for (size_t k = 0; k < target->trait_count; k++) {
                    if (strcmp(target->traits[k].name, trait_name) == 0) {
                        ...
                        civ_float_t adoption = source_strength * event->adoption_level * time_delta * 0.1f;
                        target->traits[k].strength = CLAMP(target->traits[k].strength + adoption, 0.0f, 1.0f);
                        break;
                    }
                }
                ...
                if (!found && event->adoption_level > 0.2f) {
                    ...
                    civ_cultural_identity_add_trait(target, trait_name, source_strength * event->adoption_level * 0.5f);
                }
            }
        }
...
civ_assimilation_event_t* civ_assimilation_tracker_get_event(const civ_assimilation_tracker_t* tracker,
                                                             const char* source_id, const char* target_id) {
    if (!tracker || !source_id || !target_id) return NULL;
...
civ_float_t civ_assimilation_calculate_rate(civ_assimilation_tracker_t* tracker,
                                           const civ_cultural_identity_t* source,
                                           const civ_cultural_identity_t* target,
                                           civ_assimilation_type_t type) {
    if (!tracker || !source || !target) return 0.0f;
    ...
    civ_float_t similarity = civ_cultural_identity_calculate_similarity(source, target);
