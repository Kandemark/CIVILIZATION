...
civ_cultural_identity_manager_t* civ_cultural_identity_manager_create(void) {
    civ_cultural_identity_manager_t* manager = (civ_cultural_identity_manager_t*)CIV_MALLOC(sizeof(civ_cultural_identity_manager_t));
    if (!manager) {
...
void civ_cultural_identity_manager_init(civ_cultural_identity_manager_t* manager) {
    ...
    manager->identities = (civ_cultural_identity_t*)CIV_CALLOC(manager->identity_capacity, sizeof(civ_cultural_identity_t));
}

civ_cultural_identity_t* civ_cultural_identity_create(const char* id, const char* name) {
    if (!id || !name) return NULL;
    
    civ_cultural_identity_t* identity = (civ_cultural_identity_t*)CIV_MALLOC(sizeof(civ_cultural_identity_t));
    if (!identity) {
    ...
    identity->traits = (civ_cultural_trait_t*)CIV_CALLOC(identity->trait_capacity, sizeof(civ_cultural_trait_t));
...
civ_result_t civ_cultural_identity_add_trait(civ_cultural_identity_t* identity, const char* trait_name, civ_float_t strength) {
    ...
    if (identity->trait_count >= identity->trait_capacity) {
        ...
        identity->traits = (civ_cultural_trait_t*)CIV_REALLOC(identity->traits,
    ...
    for (size_t i = 0; i < a->trait_count; i++) {
        for (size_t j = 0; j < b->trait_count; j++) {
            if (strcmp(a->traits[i].name, b->traits[j].name) == 0) {
                civ_float_t strength_diff = fabsf(a->traits[i].strength - b->traits[j].strength);
...
civ_result_t civ_cultural_identity_manager_add(civ_cultural_identity_manager_t* manager, civ_cultural_identity_t* identity) {
    ...
    if (manager->identity_count >= manager->identity_capacity) {
        ...
        manager->identities = (civ_cultural_identity_t*)CIV_REALLOC(manager->identities,
