...
#include "core/governance/legal/constitution.h"
#include <stdio.h>
#include <string.h>
#include <time.h>
...
/* ── Amendment process ──────────────────────────────────────────── */
bool civ_constitution_propose_amendment(civ_constitution_t *c, const char *rule_id,
                                        const char *new_text, float support) {
  if (!c || !rule_id) return false;
  /* Amendment can be proposed if support exceeds threshold */
  return support >= c->amendment_threshold;
}

bool civ_constitution_ratify_amendment(civ_constitution_t *c, const char *rule_id,
                                       float legislative_support) {
  if (!c || !rule_id) return false;
  if (legislative_support < c->amendment_threshold) return false;
  c->last_amendment = time(NULL);
  return true;
}

int civ_constitution_amendment_count(const civ_constitution_t *c) {
  /* Count rules that have been modified (non-zero modifier_value implies amendment) */
  if (!c) return 0;
  int count = 0;
  for (size_t i = 0; i < c->rule_count; i++)
    if (c->rules[i].modifier_value != 0.0f) count++;
  return count;
}

void civ_constitution_amend_rule(civ_constitution_t *c, const char *rule_id,
                                 const char *new_description, float new_modifier) {
  if (!c || !rule_id) return;
  civ_rule_t *r = civ_constitution_find_rule(c, rule_id);
  if (!r) return;
  if (new_description)
    strncpy(r->description, new_description, STRING_MAX_LEN - 1);
  r->modifier_value = new_modifier;
  c->last_amendment = time(NULL);
}
