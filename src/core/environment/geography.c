...
void civ_geography_init(civ_geography_t* geo, const char* region_name, civ_coordinate_t sw, civ_coordinate_t ne) {
    ...
    geo->land_patches = (civ_land_patch_t*)CIV_CALLOC(geo->patch_capacity, sizeof(civ_land_patch_t));
}

static bool is_within_bounds(const civ_geography_t* geo, civ_coordinate_t coord) {
    return (coord.latitude >= geo->bounds_sw.latitude && coord.latitude <= geo->bounds_ne.latitude &&
            coord.longitude >= geo->bounds_sw.longitude && coord.longitude <= geo->bounds_ne.longitude);
}
...
civ_result_t civ_geography_add_land_patch(civ_geography_t* geo, const civ_land_patch_t* patch) {
    ...
    if (geo->patch_count >= geo->patch_capacity) {
        ...
        geo->land_patches = (civ_land_patch_t*)CIV_REALLOC(geo->land_patches,
                                                           geo->patch_capacity * sizeof(civ_land_patch_t));
    }
...
civ_float_t civ_geography_calculate_distance(civ_coordinate_t a, civ_coordinate_t b) {
    ...
    civ_float_t a_val = sin_dlat * sin_dlat + cos(lat1) * cos(lat2) * sin_dlon * sin_dlon;
