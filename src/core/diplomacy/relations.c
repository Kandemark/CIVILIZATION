...
civ_diplomacy_system_t* civ_diplomacy_system_create(void) {
    civ_diplomacy_system_t* ds = (civ_diplomacy_system_t*)CIV_MALLOC(sizeof(civ_diplomacy_system_t));
    if (!ds) {
...
void civ_diplomacy_system_init(civ_diplomacy_system_t* ds) {
    ...
    ds->relations = (civ_diplomatic_relation_t*)CIV_CALLOC(ds->relation_capacity, sizeof(civ_diplomatic_relation_t));
    ds->treaty_capacity = 50;
    ds->treaties = (civ_treaty_t*)CIV_CALLOC(ds->treaty_capacity, sizeof(civ_treaty_t));
}

void civ_diplomacy_system_initialize_relations(civ_diplomacy_system_t* ds, const char** nation_ids, size_t nation_count) {
    if (!ds || !nation_ids) return;
        ...
        for (size_t j = 0; j < nation_count; j++) {
            if (i != j) {
                if (ds->relation_count >= ds->relation_capacity) {
                    ...
                    ds->relations = (civ_diplomatic_relation_t*)CIV_REALLOC(ds->relations, 
...
civ_diplomatic_relation_t* civ_diplomacy_system_get_relation(civ_diplomacy_system_t* ds, 
                                                              const char* nation_a, const char* nation_b) {
    if (!ds || !nation_a || !nation_b) return NULL;
...
void civ_diplomacy_system_update_relations(civ_diplomacy_system_t* ds, time_t current_date) {
    if (!ds) return;
    ...
    for (size_t i = 0; i < ds->relation_count; i++) {
        ...
        rel->trust = CLAMP(rel->trust - drift, 0.0f, 1.0f);
...
civ_result_t civ_diplomacy_system_propose_treaty(civ_diplomacy_system_t* ds, const char* proposer, 
                                                  const char* recipient, civ_treaty_type_t type, int32_t duration_days) {
    ...
    civ_diplomatic_relation_t* rel = civ_diplomacy_system_get_relation(ds, proposer, recipient);
    if (!rel) {
    ...
    if (ds->treaty_count >= ds->treaty_capacity) {
        ...
        ds->treaties = (civ_treaty_t*)CIV_REALLOC(ds->treaties, ds->treaty_capacity * sizeof(civ_treaty_t));
    }
    
    civ_treaty_t* treaty = &ds->treaties[ds->treaty_count++];
    memset(treaty, 0, sizeof(civ_treaty_t));
    
    snprintf(treaty->treaty_id, sizeof(treaty->treaty_id), "treaty_%zu", ds->treaty_count);
